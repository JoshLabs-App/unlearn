// 词元归并：walked/walking/walks → walk。逻辑与 a-decade-apart/scripts/lemma.mjs 和
// a-decade-apart/main.js 里的实现相同，改一处要同步另外两处。
// 候选词形只有在语料词表里真实存在时才采用（避免把 "tired" 砍成 "tire"）。
...
export function lemmatize(word: string, vocab?: Set<string>): string {
  ...
  const has = (c: string) => (vocab ? vocab.has(c) : true);
  const cands: string[] = [];
  const dbl = (st: string) => (/([^aeiou])\1$/.test(st) ? [st.slice(0, -1)] : []);
...
export function isStopword(lemma: string): boolean {
  return STOP.has(lemma) || lemma.length < 2;
}
